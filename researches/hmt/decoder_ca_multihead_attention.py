...
class DecoderCAMultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if saved_state is not None:
            ...
            key_padding_mask = (
                DecoderCAMultiheadAttention._append_prev_key_padding_mask(
        ...
        attn_weights = attn_weights.masked_fill(
            self.build_cand_mask(cands, src_len, tgt_len),
            float("-inf"),
        )
        ...
        attention = attn_weights.contiguous().view(
        ...
        return attn, attention  # attn_weights

    def build_cand_mask(self, cands, src_len, tgt_len):
        tmp = torch.arange(0, src_len, device="cuda").unsqueeze(0).repeat(tgt_len, 1)
        cand_mask = tmp >= (cands.unsqueeze(1))
        return cand_mask
