...
@register_model("uni_unity_conformer_waitk")
class UniUnityConformerWaitkModel(UniS2UTConformerModel):
    """
    ...
    @staticmethod
    def add_args(parser):
        UniS2UTConformerModel.add_args(parser)
        parser.add_argument(
    ...
    def forward(
        ...
        mt_decoder_out = mt_decoder(
            ...
            streaming_config={
                "src_wait": int(
                    streaming_config["k1"] * streaming_config["segment_size"] / 160
                ),
                "src_step": int(
                    streaming_config["n1"] * streaming_config["segment_size"] / 160
                ),
                "tgt_step": int(streaming_config["n1"]),
            },
        )
        ...
        if self.t2u_augmented_cross_attn:
            ...
            decoder_out = self.decoder(
                ...
                streaming_config={
                    ...
                    "tgt_step": int(
                        streaming_config["n2"] * streaming_config["unit_per_subword"]
                    ),
                },
            )
...
@register_model_architecture(
    model_name="uni_unity_conformer_waitk", arch_name="uni_unity_conformer_waitk"
)
def uni_unity_conformer_architecture_base(args):
