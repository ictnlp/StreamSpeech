...
def get_segment_waveform(
    path_or_fp: Union[str, BinaryIO], offset, length, normalization=True
    ...
    waveform, sample_rate = sf.read(
        path_or_fp, dtype="float32", start=offset, frames=length
    )
    # print(waveform)
    if not normalization:
        waveform *= 2**15
    # waveform = EXTRACTOR(waveform, return_tensors="np", sampling_rate=16000).input_values
